...
def setup_loaders(args):
    ...
    val_input_transform = standard_transforms.Compose([standard_transforms.Resize((400, 800)),
                                                       standard_transforms.CenterCrop(300),
        standard_transforms.ToTensor(),
        standard_transforms.Normalize(*mean_std)
    ])

    target_transform = [standard_transforms.Resize((400, 800), 0), standard_transforms.CenterCrop(300),
                        extended_transforms.MaskToTensor()]
    target_transform = standard_transforms.Compose(target_transform)

    # target_train_transform = extended_transforms.MaskToTensor()
    target_train_transform = [extended_transforms.MaskToTensor()]
    target_train_transform = standard_transforms.Compose(target_train_transform)
    ...
    train_loader = DataLoader(train_set, batch_size=args.train_batch_size,
                              num_workers=args.num_workers, shuffle=True, drop_last=True, sampler = train_sampler)
