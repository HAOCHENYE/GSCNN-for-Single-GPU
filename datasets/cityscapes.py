...
num_classes = 19
ignore_label = 255
root = 'C:/Users/haochenye/.mxnet/datasets'

palette = [128, 64, 128, 244, 35, 232, 70, 70, 70, 102, 102, 156, 190, 153, 153,
           153, 153, 153, 250, 170, 30,
           220, 220, 0, 107, 142, 35, 152, 251, 152, 70, 130, 180, 220, 20, 60,
           255, 0, 0, 0, 0, 142, 0, 0, 70,
           0, 60, 100, 0, 80, 100, 0, 0, 230, 119, 11, 32]    #all posible RGB value
...
#将验证集和训练集内容混杂，但是始终保证验证集中只有3个城市，训练过程中默认cv_split为0，因此训练和验证数据和数据集一致cv_split为:[{train:[train/city1, train/city2], val:[val/city1……]}]
    cv_splits = []
...
class CityScapes(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #img_name为图片原始名字(不带扩展)
        img_name = os.path.splitext(os.path.basename(img_path))[0]
        mask = np.array(mask)
        mask_copy = mask.copy()
        for k, v in id_to_trainid.items():
            #将mask中的id转换成实际训练的trainid，如cityscapes_labels.py中所示
            mask_copy[mask == k] = v
